...
const GeneralInfo = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getDocFromParams({
          slug: ["module-0-a", "beforeyoustart"],
        });

        console.log(data, " data recevied");
      } catch (err) {
        console.error(err);
      } 
    })();
  ...
  console.log(postData, " fetch krke post");
