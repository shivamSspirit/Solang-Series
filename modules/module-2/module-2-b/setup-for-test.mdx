...
Here, we create an ***custom*** spl token named S***olana pro*** which has the title G***old.*** For this first, we create the Metaplex connection using this:
...
Now we can move forward to build our program to generate program types and ids. Building, deployment and testing of the program will be explored deeply in the next lession.